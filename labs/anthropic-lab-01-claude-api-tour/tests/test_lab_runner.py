...
def test_build_lab_prompt_contains_expected_keywords() -> None:
    ...
    assert "best practice" in prompt


def test_dry_run_message_has_bullets() -> None:
    text = lab_runner.call_claude_dry("demo")
...
def test_stage1_auth_passes_in_dry_run(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
    result = lab_runner.run_auth_check(live_run=False)
    assert result.ok is True


def test_stage1_auth_fails_in_live_run_without_key(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
    result = lab_runner.run_auth_check(live_run=True)
    assert result.ok is False
    assert "ANTHROPIC_API_KEY" in result.details


def test_stage1_auth_passes_in_live_run_with_key(monkeypatch: pytest.MonkeyPatch) -> None:
...
# ---------------------------------------------------------------------------
# Stage 2 short-circuit: skipped detail when auth failed
# ---------------------------------------------------------------------------

def test_stage2_is_skipped_when_live_run_has_no_key(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
    results, _prompt, _response = lab_runner.run(stage="all", live_run=True)
    stage2 = next(r for r in results if r.stage == "stage2_first_message")
...
def test_single_stage3_stream_returns_chunks() -> None:
    ...
    assert len(results) == 1
    assert results[0].stage == "stage3_stream_simulation"
...
# ---------------------------------------------------------------------------
# Guacamole hooks: stubs must not raise
# ---------------------------------------------------------------------------
